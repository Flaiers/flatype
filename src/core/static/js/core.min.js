...
function getFigureValueByUrl(t) {
  ...
  if (
    ...
    return { embed: "/embed/youtube?url=" + encodeURIComponent(t) };
  if ((e = t.match(/^(https?):\/\/(www\.)?vimeo\.com\/(\d+)/i)))
    return { embed: "/embed/vimeo?url=" + encodeURIComponent(t) };
  if (
    (e = t.match(
      /^(https?):\/\/(www\.|mobile\.)?twitter\.com\/(.+)\/status\/(\d+)/i
    ))
  )
    return { embed: "/embed/twitter?url=" + encodeURIComponent(t) };
  if (
    (e = t.match(
      /^(https?):\/\/(t\.me|telegram\.me|telegram\.dog)\/([a-zA-Z0-9_]+)\/(\d+)/i
    ))
  )
    return { embed: "/embed/telegram?url=" + encodeURIComponent(t) };
  if (
    (e = t.match(
  ...
  $.ajax(T.apiUrl + "/save", {
    ...
    success: function (t) {
      return (
        ...
        t.error
          ? (updateEditable(!0), showError(t.error))
          : (draftClear(),
